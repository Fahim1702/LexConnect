...
test('setup creates templates once and never overwrites existing settings', async () => {
  ...
  try {
    ...
    await writeFile(join(directory, 'server/.env.example'), 'FIREBASE_PROJECT_ID=\n');
    ...
    assert.equal(serverEnv, 'FIREBASE_PROJECT_ID=\n');
    await writeFile(join(directory, 'server/.env'), 'FIREBASE_PROJECT_ID=existing-team-project\n');
    await writeFile(join(directory, 'client/.env'), 'VITE_API_URL=http://custom.example/api\n');
    assert.equal(run().status, 0);
    assert.equal(await readFile(join(directory, 'server/.env'), 'utf8'), 'FIREBASE_PROJECT_ID=existing-team-project\n');
