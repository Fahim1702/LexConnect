...
Run `npm run seed:demo` from the project root with the development database configured. Production mode is rejected. The command adds missing records using stable demo IDs and preserves existing edits on reruns. It does not erase data or replace real accounts.

Included records:

- Arif Hasan, Mira Ahmed and Samira Rahman, each labeled **Demo**, with fictional professional information and a local illustrated avatar.
- Rafi Karim, Nila Akter and Imran Chowdhury, each labeled **Demo**, with reserved `.example.test` email addresses and no real phone numbers.
- Three consultations spanning pending, assigned and resolved states.
- Three published case studies, three articles, three categorized FAQs and one explicitly fictional review.
